...
def compute_world_ray(ProjectionMatrix, WorldToCameraMatrix, screen_point, width, height):
    """计算从屏幕点到世界空间的射线点对（在投影矩阵中添加微小扰动）"""
    # 为 ProjectionMatrix 添加微小扰动
    epsilon=0
    noise = np.random.randn(*ProjectionMatrix.shape) * epsilon
    ProjectionMatrix_perturbed = ProjectionMatrix + noise

    # 1️⃣ NDC 坐标 (范围 [-1,1])
    screen_x, screen_y = screen_point["x"], screen_point["y"]
    ndc_x = (screen_x / width) * 2.0 - 1.0
    ndc_y = (screen_y / height) * 2.0 - 1.0  # 不翻转 Y
    ...
    # 2️⃣ NDC -> 相机坐标
    inv_proj = np.linalg.inv(ProjectionMatrix_perturbed)
...
def calculate_and_plot_3d_rmse(midpoints, gt_points, save_path="ex3result"):
    """
    计算3D RMSE并在三维空间中绘制路径（支持三轴等比例显示 + 固定刻度间隔）
    """
    # 计算3D RMSE
    ...
    # === 固定刻度间隔为 2.5 ===
    tick_interval = 2.5
    ax.set_xticks(np.arange(np.floor(mid_x - max_range), np.ceil(mid_x + max_range) + tick_interval, tick_interval))
    ax.set_yticks(np.arange(np.floor(mid_y - max_range), np.ceil(mid_y + max_range) + tick_interval, tick_interval))
    ax.set_zticks(np.arange(np.floor(mid_z - max_range), np.ceil(mid_z + max_range) + tick_interval, tick_interval))
...
# calculate_and_plot_3d_rmse(midpoints,gt) #双目
rightpoint=np.array(rightpoint)
calculate_and_plot_3d_rmse(rightpoint,gt)